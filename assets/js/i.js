...
function login() {         
	...
	$.post( "_system/action.php?login",{username: $('#username').val(),password: $('#password').val()},  function( data ) {
...
function register() {
	$("#return").html("<script>warning('<i class=\"fa fa-spinner fa-spin fa-fw\"></i>\', 'กรุณารอสักครู่...');</script>");
	$.post( "_system/action.php?register",{
...
function redeem() {
	$("#return").html("<script>warning('<i class=\"fa fa-spinner fa-spin fa-fw\"></i>\', 'กรุณารอสักครู่...');</script>");
	$.post( "_system/action.php?redeem",{code: $('#code').val()},  function( data ) {
	$("#btn").prop("disabled", true);
	$( "#return" ).html( data );
	$("#btn").prop("disabled", false); 
  }
...
function password() {
	$("#return").html("<script>warning('<i class=\"fa fa-spinner fa-spin fa-fw\"></i>\', 'กรุณารอสักครู่...');</script>");
	$.post( "_system/action.php?cpassword",{
...
function buy(id) {
	$("#return").html("<script>warning('<i class=\"fa fa-spinner fa-spin fa-fw\"></i>\', 'กรุณารอสักครู่...');</script>");
	$.get( "_system/action.php?buy&id="+id,  function( data ) {
	$("#btn").prop("disabled", true);
	$( "#return" ).html( data );
	$("#btn").prop("disabled", false); 
  }
...
function ibuyitem(id) {
	...
	$.get( "_system/action.php?shop&id="+id, function( data ) {
	$( "#return" ).html( data );
  }
 );
}
function code() {
	$("#return").html("<script>warning('<i class=\"fa fa-spinner fa-spin fa-fw\"></i>\', 'กำลังทำรายการ รอสักครู่');</script>");
	$.post( "_system/action.php?code",{code: $('#code').val()}, function( data ) {
	$("#btn").prop("disabled", true);
	$( "#return" ).html( data );
	$("#btn").prop("disabled", false); 
  }
...
function oc(id){
  $.get("_system/action.php?category="+id, function(data, status){
  	$( "#return" ).html( data );
  });
}
function osay(){
...
{
	var id = 1;
   $.get("_system/action.php?osay="+id, function(data, status){
  	$( "#return" ).html( data );
  });
}
else
...
function udelete(id) {
	swal({
		...
		html: '<br><br>' +
		'<a class="float-left btn btn-danger text-light" onclick="swal.close()"><i class="fa fa-times"></i> ยกเลิก</a>' +
		'<a class="float-right btn btn-success" href="?page=user&users&delete&number='+id+'"><i class="fa fa-check"></i> ยืนยัน</a>',
	});
}
